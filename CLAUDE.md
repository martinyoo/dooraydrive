...
- **배포 경로는 있는데 갱신 경로가 없다.** `설치.bat`은 기존 설치를 발견하면
  `Already present - reusing it`으로 건너뛴다([설치.bat:127](설치.bat:127)) — 재실행해도 코드가 갱신되지 않는다.
...
- 구현 항목 A~D: 버전 정체성(`__version__`이 `0.1.0`에 멈춰 있고 어디에도 표시되지 않는다) ·
  `설치.bat` 갱신 모드 · `synchere.bat` 새 버전 알림 · DB `PRAGMA user_version`
  (지금 스키마 버전이 없어 컬럼을 추가하는 릴리스는 기존 PC에서 조용히 깨진다).
